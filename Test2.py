...
# Update obstacles parameters to be within track and add spikes
max_obstacles = 15
...
def generate_obstacle():
    global inner_radius, outer_radius
    obstacle_types = ['boulder', 'mud', 'spikes']
    obs_type = random.choice(obstacle_types)

    # Generate random position within track boundaries
    angle = math.radians(random.uniform(0, 360))
    radius = random.uniform(inner_radius + 2, outer_radius - 2)

    obstacle = {
        'type': obs_type,
        'pos': [
            radius * math.cos(angle),
            0.02,  # Keep slightly raised position
            radius * math.sin(angle)
        ],
        'radius': random.uniform(0.5, 1.5) if obs_type == 'boulder' else
                 random.uniform(1.2, 2.0) if obs_type == 'mud' else  # Reduced mud size
                 random.uniform(1.5, 3.0),
        'created': time.time()
    }

    # Pre-calculate spike positions if it's a spike trap
    if obs_type == 'spikes':
        obstacle['spike_data'] = []
        spike_count = random.randint(5, 9)
        for _ in range(spike_count):
            angle = math.radians(random.uniform(0, 360))
            dist = random.uniform(0, obstacle['radius'])
            height = random.uniform(0.8, 1.5)
            obstacle['spike_data'].append((
                math.cos(angle) * dist,
                math.sin(angle) * dist,
                height
            ))
...
def draw_obstacles():
    for obs in obstacles:
        ...
        if obs['type'] == 'boulder':
            ...
            glColor3f(0.5, 0.4, 0.2)  # Maintain bright color
            size = obs['radius']
            ...
            for i in range(32+1):
                ...
                x = math.cos(angle) * size * 0.9  # Create slight irregularity
                z = math.sin(angle) * size * 0.9
                glVertex3f(x, 0.02, z)
            glEnd()
            # Add dark border
            ...
            for i in range(32):
                ...
                glVertex3f(x, 0.025, z)
            glEnd()

        elif obs['type'] == 'spikes':
            glColor3f(0.8, 0.1, 0.1)  # Bright red spikes
            for spike in obs['spike_data']:
                x, z, height = spike
                glPushMatrix()
                glTranslatef(x, 0, z)
                glRotatef(-90, 1, 0, 0)
                glutSolidCone(0.1, height, 6, 6)
                glPopMatrix()
        glPopMatrix()
...
def idle():
    ...
    global accel, heading, yrot, out, last_spawn_time, obstacles, max_obstacles
    global spawn_interval  # MOVE THIS LINE UP HERE
    ...
    if race_started:
        ...
        if len(obstacles) < max_obstacles and current_time - last_spawn_time > spawn_interval:
            obstacles.append(generate_obstacle())
            last_spawn_time = current_time
            # Gradually increase spawn rate
            spawn_interval = max(0.5, spawn_interval * 0.95)  # REMOVE GLOBAL DECLARATION HER

    # Obstacle lifetime management (optional)
    obstacles = [obs for obs in obstacles if time.time() - obs['created'] < 30]
