...
def draw_wheel():
    """Draw a single wheel using a cylinder for the outer rim and a sphere for the hub"""
    glPushMatrix()

    # Position the wheel correctly (rotate around Y-axis to make it 90 degrees rotated)
    glRotatef(90, 0, 1, 0)  # Rotate the wheel 90 degrees around Y-axis

    # Outer rim of the wheel (using a cylinder)
    glColor3f(0.2, 0.2, 0.1)  # Black color for the wheel
    glPushMatrix()
    glutSolidCylinder(0.2, 0.1, 16, 8)  # Cylinder with radius 0.2 and height 0.1
    glPopMatrix()

    # Hub of the wheel (using a sphere)
    glColor3f(0.3, 0.3, 0.3)  # Slightly lighter color for the hub
    glPushMatrix()
    glTranslatef(0.0, 0.0, 0.05)  # Slightly offset to center it on the rim
    glutSolidSphere(0.08, 16, 8)  # Hub with radius 0.08
    glPopMatrix()
...
def draw_cpu_car(x, z, direction="forward", color=(0.0, 0.0, 1.0)):
    """Draw a CPU car with the same model as the player car but different color"""
    glPushMatrix()
    ...
    # Wheels (symmetrical placement for both left and right wheels)
    wheel_offset = 0.6  # Distance of wheels from the center of the car along the X-axis
    wheel_height = -0.3  # Height of the wheels, placed beneath the car

    # Adjust the right wheels slightly to the left (make them closer to the center)
    right_wheel_offset = wheel_offset - 0.1  # Move the right wheels 0.1 units to the left
    ...
    # Front-right wheel (slightly moved left)
    glPushMatrix()
    glTranslatef(right_wheel_offset, wheel_height, 0.7)  # Position right front wheel (closer to the center)
    draw_wheel()
    glPopMatrix()

    # Rear-left wheel
    glPushMatrix()
    glTranslatef(-wheel_offset, wheel_height, -0.7)  # Position left rear wheel
    draw_wheel()
    glPopMatrix()

    # Rear-right wheel (slightly moved left)
    glPushMatrix()
    glTranslatef(right_wheel_offset, wheel_height, -0.7)  # Position right rear wheel (closer to the center)
    draw_wheel()
    glPopMatrix()
...
def draw_car():
    """Draw a simple car model with wheels placed symmetrically, adjusting the right wheels"""
    glPushMatrix()

    # Position and rotate the car
    glTranslatef(car_x, car_y + 0.5, car_z)  # Position the car in 3D space (+0.5 to raise the car slightly)
    glRotatef(car_rotation, 0, 1, 0)  # Rotate the car based on its direction
...
def update_car():
    ...
    if not game_over:
        ...
        if move_forward:
            ...
            if car_speed > 0:  # Only allow reverse when the car is already moving forward
                car_speed -= acceleration
                if car_speed < 0:
                    car_speed = 0  # Ensure the speed never goes negative
        else:
            # Apply friction (deceleration when no keys are pressed)
            if car_speed > 0:
                car_speed -= deceleration
                if car_speed < 0:
                    car_speed = 0  # Ensure the speed doesn't go below 0
            elif car_speed < 0:
                car_speed = 0  # If speed is negative, reset to 0
        ...
        # Check for collision with road borders and game over if wheels touch walls
        wheel_offset = 0.6  # Distance of wheels from the center of the car along the X-axis
        if abs(car_x - wheel_offset) > road_width / 2 or abs(car_x + wheel_offset) > road_width / 2:
            game_over = True  # Game ends if either wheel crosses the road boundary

        # Collision check with CPU cars
        for car in cpu_cars:
            # Check if the player's car is close enough to the CPU car (based on X, Z positions)
            if (abs(car.x - car_x) < 1.0 and abs(car.z - car_z) < 1.0):  # Adjust distance threshold as needed
                # Check if the player's wheel intersects with the CPU car's wheel
                ...
                # Check for collision between player's left wheel and CPU's left wheel
                if abs(player_left_wheel - cpu_left_wheel) < 0.2 and abs(car.z - car_z) < 1.0:
                    game_over = True  # Game ends if the player's left wheel collides with CPU's left wheel

                # Check for collision between player's right wheel and CPU's right wheel
                if abs(player_right_wheel - cpu_right_wheel) < 0.2 and abs(car.z - car_z) < 1.0:
                    game_over = True  # Game ends if the player's right wheel collides with CPU's right wheel
...
def update_cpu_cars():
    ...
    if score < 5000:
        cpu_spawn_interval = 350  # Slow spawn rate for early stages (longer interval)
    elif score < 10000:
        cpu_spawn_interval = 300  # Slightly faster as score increases
    elif score < 15000:
        cpu_spawn_interval = 250  # Moderate spawn rate
    else:
        cpu_spawn_interval = 180  # Fastest spawn rate when score is high (can be adjusted)

    # Start with fewer CPU cars and increment slower
    if score < 10000:
        # Start with 1 car and increase by 1 car every 5000 score points
        num_cars = 1 + (score // 5000)  # Start with 1 car and increase by 1 car every 5000 score points
        if num_cars > 2:
            num_cars = 2  # Cap the number of cars at 2, you can change this value if you want a higher cap
    else:
        # Once score reaches 10000, keep the number of cars at the maximum
        num_cars = 2
    ...
    if cpu_spawn_timer >= cpu_spawn_interval:
        ...
        lanes = [-road_width / 4, 0, road_width / 4]  # Left, center, and right lanes

        for _ in range(num_cars):
            # Try to spawn a car in a valid position without overlap
            car_spawned = False
            while not car_spawned:
                ...
                min_distance = 1.5  # Minimum distance to prevent overlap
                overlap = False
                for car in cpu_cars:
